...
function Sticker({
  ...
  const [initialRotation] = useState<number>(
    fixedRotation !== undefined ? fixedRotation : getRandomNumberInRange(-15, 15),
  );
...
export function ScrapbookBento({ className }: { className?: string }) {
  ...
  return (
    <BentoCard
      ...
      hideOverflow={false}
    >
      <h2 className="mb-2 font-medium">Certificates</h2> {/* Changed title to Certificates */}
      ...
      >
        <motion.div
          ...
          initial="hidden"
          animate="shown"
          className="-mt-8 grid h-full w-full grid-cols-3 items-center gap-4" // Changed grid-cols-4 to grid-cols-3 for 3 certificates
        >
          {/* Certificate 1 */}
          <Sticker
            fixedRotation={-5} // Fixed rotation
            fixedY={-10}    // Fixed Y offset
            // No caption needed for certificates
          >
            <img
              width="100"
              src="/static/certificate1.png" // Your certificate image path
              alt="Certificate 1"
              className="xs:max-w-none max-w-[120px] rounded-md shadow-md"
              draggable={false}
            />
          </Sticker>
          {/* Certificate 2 */}
          <Sticker
            fixedRotation={5} // Fixed rotation
            fixedY={5}     // Fixed Y offset
            // No caption needed for certificates
          >
            <img
              width="100"
              src="/static/certificate2.png" // Your certificate image path
              alt="Certificate 2"
              className="xs:max-w-none max-w-[120px] rounded-md shadow-md"
              draggable={false}
            />
          ...
          {/* Certificate 3 */}
          <Sticker
            fixedRotation={-10} // Fixed rotation
            fixedY={-15}     // Fixed Y offset
            // No caption needed for certificates
          >
            <img
              width="100"
              src="/static/certificate3.png" // Your certificate image path
              alt="Certificate 3"
              className="xs:max-w-none max-w-[120px] rounded-md shadow-md"
              draggable={false}
            />
