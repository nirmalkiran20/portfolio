  ...
  schema: s
    .object({
      ...
      slug: s.custom().transform((_, { meta }) => {
        // Fix: Cast meta to include basename
        const fileMeta = meta as { basename?: string };
        return fileMeta.basename?.replace(/\.mdx$/, "") || "";
      }),
      code: s.mdx(),
...
export default defineConfig({
  ...
  mdx: {
    rehypePlugins: [
      [rehypeRaw, { passThrough: ['mdxTextExpression', 'mdxJsxFlowElement', 'mdxJsxTextElement', 'mdxFlowExpression'] }]
    ],
    remarkPlugins: [],
  },
});
